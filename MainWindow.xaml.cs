...
public partial class MainWindow : Window
{
    ...
    {
        var c = new HttpClient(new SocketsHttpHandler { AutomaticDecompression = System.Net.DecompressionMethods.All });
    ...
    // JSON models
    private sealed class GeoResp { [JsonPropertyName("results")] public List<GeoResult>? Results { get; set; } }
    ...
    private sealed class ForecastResp
    {
    ...
    private sealed class ReverseGeoResp
    {
    ...
    // ---------- State ----------
    private record WidgetConfig(int X, int Y, string? Prov, string? City);
    ...
    private void Window_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        var refresh = new System.Windows.Threading.DispatcherTimer { Interval = TimeSpan.FromMinutes(10) };
        refresh.Tick += async (_, _) => await FetchWeatherAsync();
        refresh.Start();
    ...
    private void LoadCityConfig()
    {
        try
        {
            ...
            {
                var cfg = JsonSerializer.Deserialize<WidgetConfig>(File.ReadAllText(ConfigPath));
    ...
    private void SaveConfig()
    {
        try
        {
            ...
            File.WriteAllText(ConfigPath, JsonSerializer.Serialize(
                new WidgetConfig((int)Left, (int)Top, _currentProv, _currentCity)));
        }
        catch { }
    }
    ...
    private Border MakeMenuItem(string text, bool active, bool small, MouseButtonEventHandler onClick)
    {
        ...
        var label = new TextBlock
        {
            ...
            Foreground = active ? Brushes.White : new SolidColorBrush(Color.FromArgb(204, 255, 255, 255))
        };
        border.Child = label;
    ...
    private async Task<(double Lat, double Lon)?> GetCoordsAsync(string city)
    {
        ...
        try
        {
            ...
            var data = await JsonSerializer.DeserializeAsync<GeoResp>(
    ...
    private async Task FetchWeatherAsync()
    {
        ...
        var coords = await GetCoordsAsync(_currentCity);
        if (coords == null) { ErrorText.Text = "无法获取城市坐标"; return; }
    ...
    private async Task FetchWeatherWithCoordsAsync((double Lat, double Lon) coords)
    {
        try
        {
            ...
            using var resp = await Http.GetAsync(url);
            resp.EnsureSuccessStatusCode();
            var data = await JsonSerializer.DeserializeAsync<ForecastResp>(
                await resp.Content.ReadAsStreamAsync(), JsonOpts);
    ...
    // ---------- Particles ----------
    private static void ClearLayer(Canvas layer)
    {
        foreach (var child in layer.Children.OfType<FrameworkElement>().ToList())
        {
            ...
            switch (child.RenderTransform)
            {
                ...
                case TransformGroup g:
                    ...
                    {
                        if (t is RotateTransform r) r.BeginAnimation(RotateTransform.AngleProperty, null);
                        if (t is TranslateTransform tr) tr.BeginAnimation(TranslateTransform.YProperty, null);
                    }
                    break;
            }
        }
    ...
    {
        ClearLayer(RainCanvas);
        ClearLayer(SnowCanvas);
        ClearLayer(StarsCanvas);

        if (type is "rainy" or "storm")
        {
            var n = type == "storm" ? 45 : 30;
            for (int i = 0; i < n; i++)
            {
                var drop = new Rectangle { Width = 1.5, Height = 12, Fill = RainGrad };
                Canvas.SetLeft(drop, Rnd.NextDouble() * 280);
                Canvas.SetTop(drop, 0);
                var tt = new TranslateTransform { Y = -12 };
                drop.RenderTransform = tt;

                var dur = 0.4 + Rnd.NextDouble() * 0.8;
                var fall = new DoubleAnimation(-12, 380, TimeSpan.FromSeconds(dur))
                {
                    BeginTime = TimeSpan.FromSeconds(Rnd.NextDouble() * 2),
                    RepeatBehavior = RepeatBehavior.Forever
                };
                var opac = new DoubleAnimationUsingKeyFrames
                {
                    Duration = TimeSpan.FromSeconds(dur),
                    BeginTime = fall.BeginTime,
                    RepeatBehavior = RepeatBehavior.Forever
                };
                opac.KeyFrames.Add(new LinearDoubleKeyFrame(0, KeyTime.FromTimeSpan(TimeSpan.Zero)));
                opac.KeyFrames.Add(new LinearDoubleKeyFrame(1, KeyTime.FromTimeSpan(TimeSpan.FromSeconds(dur * 0.1))));
                opac.KeyFrames.Add(new LinearDoubleKeyFrame(0.15, KeyTime.FromTimeSpan(TimeSpan.FromSeconds(dur))));
                drop.BeginAnimation(UIElement.OpacityProperty, opac);
        ...
        if (type == "snowy")
        {
            for (int i = 0; i < 18; i++)
            {
                var size = 3 + Rnd.NextDouble() * 5;
                var flake = new Ellipse { Width = size, Height = size, Fill = Brushes.White };
                Canvas.SetLeft(flake, Rnd.NextDouble() * 280);
                ...
                flake.RenderTransform = new TransformGroup
                {
                    Children =
                    {
                        new TranslateTransform { Y = -10 },
                        new RotateTransform()
                    }
                };

                var dur = 3 + Rnd.NextDouble() * 4;
                var begin = TimeSpan.FromSeconds(Rnd.NextDouble() * 3);
                var group = (TransformGroup)flake.RenderTransform;
                var tt = (TranslateTransform)group.Children[0];
                var rt = (RotateTransform)group.Children[1];

                var fall = new DoubleAnimation(-10, 380, TimeSpan.FromSeconds(dur))
                {
                    BeginTime = begin,
                    RepeatBehavior = RepeatBehavior.Forever
                };
                var spin = new DoubleAnimation(0, 360, TimeSpan.FromSeconds(dur))
                {
                    BeginTime = begin,
                    RepeatBehavior = RepeatBehavior.Forever
                };
                ...
                opac.KeyFrames.Add(new LinearDoubleKeyFrame(0, KeyTime.FromTimeSpan(TimeSpan.Zero)));
                opac.KeyFrames.Add(new LinearDoubleKeyFrame(0.8, KeyTime.FromTimeSpan(TimeSpan.FromSeconds(dur * 0.1))));
                opac.KeyFrames.Add(new LinearDoubleKeyFrame(0.2, KeyTime.FromTimeSpan(TimeSpan.FromSeconds(dur))));

                flake.BeginAnimation(UIElement.OpacityProperty, opac);
                tt.BeginAnimation(TranslateTransform.YProperty, fall);
        ...
        if (type == "night")
        {
            for (int i = 0; i < 22; i++)
            {
                var star = new Ellipse { Width = 2, Height = 2, Fill = Brushes.White, Opacity = 0.1 };
                Canvas.SetLeft(star, Rnd.NextDouble() * 280);
                Canvas.SetTop(star, Rnd.NextDouble() * 228);
                var twinkle = new DoubleAnimation(0.1, 0.6, TimeSpan.FromSeconds(1.5 + Rnd.NextDouble() * 2))
                {
    ...
    private async Task<(double Lat, double Lon)> GetGpsPositionAsync()
    {
        ...
        var op = geolocator.GetGeopositionAsync(TimeSpan.Zero, TimeSpan.FromSeconds(9));
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        var result = await op.AsTask(cts.Token);
    ...
    private async Task<string> ReverseGeocodeAsync(double lat, double lon)
    {
        try
        {
            ...
            using var resp = await Http.GetAsync(url);
            resp.EnsureSuccessStatusCode();
            var data = await JsonSerializer.DeserializeAsync<ReverseGeoResp>(
                await resp.Content.ReadAsStreamAsync(), JsonOpts);
    ...
    private async Task LocateDeviceAsync(bool isStartup)
    {
        ...
        try
        {
            var coords = await GetGpsPositionAsync();
            _cachedCoords = coords;
            var name = await ReverseGeocodeAsync(coords.Lat, coords.Lon);
            _currentProv = "";
            _currentCity = $"当前位置（{name}）";
            SaveConfig();
            CityBtn.Content = "📍 " + name + " ▾";
        ...
        {
            if (!isStartup) SetLocLoading(false);
        }
    }
}
